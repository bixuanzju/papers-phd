...
(* Incomplete *)
Inductive sub2 : list DExp -> list (var * DExp) -> TExp -> DExp -> DExp -> TExp -> Prop :=
| SVar : forall S1 S2 e x, sub2 S1 S2 e (DFVar x) (DFVar x) e
| SStar : forall S e, sub2 nil S e DStar DStar e
| SPi : forall L A B A' B' e e1 e2 S,
    (forall x, x \notin L ->
          sub2 nil S (TFVar x) A' A e1 /\
          sub2 nil S (TApp e e1) (B ^ x) (B' ^ x) (open e2 (TFVar x))) ->
    sub2 nil S e (DPi A B) (DPi A' B') (TLam e2)
| SLam: forall L e A B e' T D S,
    (forall x, x \notin L -> sub2 D ((x, T) :: S) (TCastdn e) (A ^ x) (B ^ x) e') ->
    sub2 (T :: D) S e (DLam A) (DLam B) (TCastup e')
| SApp: forall T A B e e' D S,
    sub2 (T :: D) S e A B e' -> sub2 D S e (DApp A T) (DApp B T) e'.
...
Section sub_ind'.
  ...
  Hypothesis pi_case : forall L A B A' B' e e1 e2 S,
      (forall x, x \notin L ->
            ...
            sub2 nil S (TApp e e1) (B ^ x) (B' ^ x) (open e2 (TFVar x))) ->
      (forall x, x \notin L ->
            P nil S (TFVar x) A' A e1 /\
            P nil S (TApp e e1) (B ^ x) (B' ^ x) (open e2 (TFVar x))) ->
  ...
  Hypothesis app_case : forall T e A B e' D S,
      sub2 (T :: D) S e A B e' -> P (T :: D) S e A B e' -> P D S e (DApp A T) (DApp B T) e'.
  ...
  Fixpoint sub2_ind' (l : list DExp) (l0 : list (var * DExp)) (t : TExp) (d d0 : DExp) (t0 : TExp) (s : sub2 l l0 t d d0 t0) {struct s} : P l l0 t d d0 t0 :=
    match s in (sub2 l1 l2 d3 d4 d5 d6) return (P l1 l2 d3 d4 d5 d6) with
    ...
    | @SLam L e A B e' T D S0 s0 => @lam_case L e A B e' T D S0 s0
                                             (fun x (y : x \notin L) =>
                                                sub2_ind' (s0 x y))
    | @SApp T e A B e' D S0 s0 => app_case s0 (sub2_ind' s0)
...
| RBNil : forall e e' A B S,
    (forall Gamma Gamma' a b,
        ...
        elab_typing Gamma (substS S A) DStar a ->
        elab_typing Gamma (substS S B) DStar b ->
        has_type Gamma' e a ->
        has_type Gamma' e' b) ->
...
Lemma apps_lemma : forall {D2 D1 S E A B E'},
    sub2 (D1 ++ D2) S E A B E' -> Result (D1 ++ D2) S E A B E' -> Result D2 S E (apps A D1) (apps B D1) E'.
Proof.
  destruct D2; intros.
  ...
  lets : (elab_unique H1 H2). subst. auto.
  (* Lam case *)
  inverts H.
  pick_fresh x.
  assert (x \notin L0) by auto.
  ...
  assert (exists c, elab_typing Gamma (substS ((x, T) :: S) (apps (B ^ x) D)) DStar c). admit. (* I think this can be derived from the substitution lemma *)
  assert (substS S (apps (DLam C) (T :: D)) ~~> substS ((x, T) :: S) (apps (C ^ x) D)). admit. (* should be derived from beta-reduction *)
  assert (exists c, elab_typing Gamma (substS ((x, T) :: S) (apps (C ^ x) D)) DStar c). admit. (* I think this can be derived from the substitution lemma *)
...
Lemma weaken_Gamma : forall {Gamma B T e}, elab_typing Gamma B T e -> 
  forall {x A A' S1 S2 e1}, binds x A Gamma -> sub2 S1 S2 (TFVar x) A' A e1 -> elab_typing Gamma B T ([x |~> e1] e).
intros Gamma B T e tB.
induction tB; intros; simpl.
Admitted.

Lemma sub_gen : forall {e e' B C D S}, sub2 D S e B C e' -> Result D S e B C e'.
Proof.
  intros.
  ...
  assert (forall E, substS S E = E). admit.
  rewrite (H5 _) in H2. rewrite (H5 _) in H3. clear H5.
  (* end: simplify *)
  ...
  clear H H0 H5 H6 H7 H11 H13 Fry L L1 L0.
