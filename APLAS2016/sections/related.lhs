...
\section{Application of Iso-Types}\label{sec:fun}
\name is a simple core calculus, but expressive enough to encode 
useful language constructs. We have implemented a simple functional
language \sufcc to show how features of modern functional
languages can be encoded in \name. We focus on common features
available in traditional functional languages and some interesting
type-level features, but not the \emph{full power} of dependent
types. Supported features include algebraic datatypes, records,
higher-kinded types, kind polymorphism~\cite{fc:pro} and datatype
promotion~\cite{fc:pro}.

Due to lack of space, many examples illustrating the various language
features supported in \sufcc are provided only in the extended version~\cite{full}.
%formalization and proofs for \sufcc
%are provided in the appendix (see Section \ref{sec:app},
%\ref{sec:surface} and \ref{sec:app:sufcc}).
Here we show the essential
idea of how to exploit iso-types to encode language
constructs.

\paragraph{Encoding Parametrized Algebraic Datatypes with Weak Casts}
We give an example of encoding parametrized algebraic datatypes in
\ecore via $\mu$-operator and \emph{weak} casts. Importantly we should
note that having iso-recursive types alone (and alpha equality) would be
insufficient to encode parametrized types: the generalization afforded
by iso-types is needed here. In \sufcc we can define \emph{polymorphic list}
as:

< data List a = Nil | Cons a (List a);

This \sufcc definition is translated into \ecore using a Scott encoding~\cite{encoding:scott}
of datatypes:

< List  =  /L : * -> * . \a : * . #b : * . b -> (a -> L a -> b) -> b
< Nil   =  \a: *  .  castup2 [List a] (\b: * . \n:b . \c:(a -> List a -> b) . n)
< Cons  =  \a: *  .  \x : a . \(xs : List a) .
<                    castup2 [List a] (\b: * . \n:b . \c:(a -> List a -> b) . c x xs)

The type constructor |List| is encoded as a recursive type. The body
is a \emph{type-level function} that takes a type parameter $a$ and
returns a dependent function type, i.e., $\Pi$-type.
The body of $\Pi$-type is universally
quantified by a type parameter $b$, which represents the result type
instantiated during pattern matching. Following are the types
corresponding to data constructors: |b| for |Nil|, and |a -> L a -> b|
for |Cons|, and the result type $b$ at the end.
The data constructors |Nil| and |Cons| are encoded as functions. Each
of them selects a different function from the parameters ($n$ and
$c$). This provides branching in the process flow, based on the
constructors. Note that |castup| is used twice here (written as |castup2|): one for \emph{one-step
  expansion} from $\tau$ to $[[(\aa:s.t) aa]]$ and the other for
\emph{folding the recursive type} from $[[(\aa:s.t) aa]]$ to |List a|,
where $\tau$ is the type of |castup2| body.

We have two notable remarks from the example above. First, iso-types
are critical for the encoding and cannot be
replaced by iso-recursive types. Since type constructors are
parameterized, not only folding/unfolding recursive types, but also
type-level reduction/expansion is required, which is only possible
with casts. Second, though weak casts are not as powerful as full
casts, they are capable of encoding many useful constructs, such as
algebraic datatypes and records~\cite{full}. Nevertheless
full-reduction casts enable other important applications.
Some applications of full casts are discussed in the extended version~\cite{full}.

%Since \ecore has a
%simpler semantics than \namef, \sufcc preferably uses \ecore for
%elaboration.
...
\name still lacks support for some features of GHC Haskell.
\end{comment}

Girard's System $F_{\omega}$~\cite{systemfw} is a typed
lambda calculus with higher-kinded polymorphism.
For the well-formedness of type expressions, an extra level of
\emph{kinds} is added to the system. In comparison, because of unified
syntax, \name is
considerably simpler than System $F_{\omega}$, both in terms of
language constructs and complexity of proofs. As for type-level computation, System $F_{\omega}$
differs from \name in that it uses a conversion rule, while \name
uses explicit casts. The
current core language for GHC Haskell, System
$F_{C}$~\cite{fc} is a significant extension of System
$F_{\omega}$, which supports GADTs~\cite{haskellgadt}, functional
dependencies~\cite{Jones00}, type families~\cite{Eisenberg:2014}, and
kind equality~\cite{fc:kind}. These features use a
non-trivial form of type equality, which is currently missing from
\name. On the other hand, \name uses unified syntax and has only 8
language constructs, whereas System $F_C$ uses multiple
levels of syntax and currently has over 30 language constructs,
...
\textsf{Zombie}~\cite{zombie:popl14} is a dependently
typed language using a single syntactic category. It is composed of
two fragments: a logical fragment where every expression is known to
terminate, and a programmatic fragment that allows general
recursion. Though Zombie has one syntactic category, it is still
fairly complicated (with around 24 language constructs) as it tries to
be both consistent as a logic and pragmatic as a programming
language. Even if one is only interested in modeling a programmatic
fragment, additional mechanisms are required to ensure
the validity of proofs, e.g., call-by-value semantics and value
restriction~\cite{sjoberg:msfp12,zombie:popl15}. In contrast to
Zombie, \name takes another point of the design space, giving up
logical consistency and reasoning about proofs for simplicity in the language
design. 

\paragraph{Unified syntax with general recursion and undecidable type
checking} Cayenne~\cite{cayenne}
integrates the full power of dependent types with general
recursion, which bears some similarities with \name. It
uses one syntactic form for both terms and types, allows
arbitrary computation at type level and is logically
inconsistent because of allowing unrestricted
recursion. However, the most crucial difference from \name is that
type checking in Cayenne is \emph{undecidable}. From a pragmatic point of
...
to implement equi-recursive types. Recursion and recursive types are
unified in a single
construct. However, both equi-recursive types and the Type:Type axiom
make the type system undecidable. $\Pi\Sigma$~\cite{dep:pisigma} is
another example of a language that uses one recursion mechanism for both types and
functions. The type-level recursion is controlled by lifted types and
boxes since definitions
are not unfolded inside boxes. However,
$\Pi\Sigma$ does not have decidable type checking due to the
``type-in-type'' axiom. And its metatheory is not formally developed.
...
fc, Doorn:2013hq} also attempt to use explicit casts for managed type-level
computation.
However, casts in those approaches are not inspired by iso-recursive types.
Instead they require \emph{equality proof terms}, while casts in \name do not. The need for
...
Guru~\cite{guru} and $\mathrm{Sep3}$~\cite{kimmel:plpv} make
\emph{syntactic separation} between proofs and programs to prevent certain
programmatic terms turning into invalid proofs. The programmatic part of
Zombie~\cite{sjoberg:msfp12, zombie:popl15}, which has no such
separation, employs \emph{value restriction} that restricts proofs to
be syntactic values to avoid non-terminating terms. PTS with
convertibility proofs ($\mathrm{PTS}_f$)~\cite{Doorn:2013hq} extends
PTS by replacing the implicit conversion rule with explicit conversion
proofs embedded into terms. However, it requires many language
constructs to build equality proofs; and it does not allow general
recursion, thus does not need to deal with problem 2).
Our treatment of full casts in \namef, using a separate erased
system for developing metatheory, is similar to the approach of
Zombie or Guru which uses an unannotated system.

\paragraph{Restricted recursion with termination checking}

As proof assistants, dependently typed languages such as
Coq~\cite{coqsite} and Adga~\cite{agda} are conservative as to what
kind of computation is allowed. They require all programs to
terminate by means of a termination checker, ensuring recursive calls
are decreasing. Decidable type checking and logical
consistency are preserved. But the conservative, syntactic criteria is
insufficient to support a variety of important programming paradigms.
Agda offers an option to disable the termination checker to
allow writing arbitrary functions. However, this may endanger both
decidable type checking and logical consistency. Idris~\cite{idris} is
a dependently typed language that allows writing unrestricted
functions. However, to achieve decidable type checking, it also
requires termination checker to ensure only terminating functions are
evaluated by the type checker. While logical consistency is an
appealing property, it is not a goal of \name. Instead \name aims at
retaining (term-level) general recursion as found in languages like
Haskell or ML, while benefiting from a unified syntax to simplify the
