(***************************************************************************
* CoC + Mu + Type-in-type - Infrastructure                                 *
* Based on CoC formalization in LibLN by                                   *
*   Arthur Chargueraud, April 2007                                         *
***************************************************************************)

Set Implicit Arguments.
Require Import LibLN CoCMu_Definitions.
...
Fixpoint subst (z : var) (u : trm) (t : trm) {struct t} : trm :=
  ...
  | trm_mu t1 t2   => trm_mu (subst z u t1) (subst z u t2)
  end.
...
Hint Constructors beta star_ equiv_ less typing wf value reduct.
Hint Unfold conv.
...
Proof.
  ...
  apply_fresh* term_mu as y. rewrite* subst_open_var.
Qed.
...
Lemma term_mu1 : forall t2 t1,
  term (trm_mu t1 t2) -> term t1.
Proof.
  intros. inversion* H.
Qed.

Lemma body_mu2 : forall t1 t2,  
  term (trm_mu t1 t2) -> body t2.
Proof.
  intros. unfold body. inversion* H.
Qed.
...
Hint Extern 1 (term ?t) =>
  ...
  | H: term (trm_mu t ?t2) |- _ => apply (@term_mu1 t2)
  end.
...
Lemma term_mu_prove : forall t1 t2,
  term t1 -> body t2 -> term (trm_mu t1 t2).
Proof.
  intros. apply_fresh* term_mu as x.
Qed.

Hint Resolve term_abs_prove term_prod_prove term_mu_prove.
...
Hint Extern 1 (term (trm_mu ([?x ~> ?u]?t1) ([?x ~> ?u]?t2))) =>
  match goal with H: term (trm_mu t1 t2) |- _ => 
...
Proof.
  ...
  pick_fresh y. lets: (IHTyp H3). notin_simpl. auto. apply* (@fv_open_var y).
