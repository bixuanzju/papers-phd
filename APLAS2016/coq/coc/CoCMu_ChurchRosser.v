(***************************************************************************
* CoC + Mu + Type-in-type - Church-Rosser Property                         *
* Based on CoC formalization in LibLN by                                   *
*   Arthur Chargueraud, April 2007                                         *
***************************************************************************)

Set Implicit Arguments.
Require Import LibLN CoCMu_Definitions CoCMu_Infrastructure CoCMu_BetaStar.
...
Inductive para : relation :=
  ...
  | para_mured : forall L t1 t1' t2 t2',
      para t1 t1' ->
      (forall x, x \notin L -> para (t2 ^ x) (t2' ^ x)) ->
      para (trm_mu t1 t2) (t2' ^^ (trm_mu t1' t2'))
  | para_mu : forall L t1 t1' t2 t2',
     para t1 t1' -> 
     (forall x, x \notin L -> para (t2 ^ x) (t2' ^ x) ) ->
     para (trm_mu t1 t2) (trm_mu t1' t2').
...
Hint Resolve para_var para_star para_app.
...
Proof.
  ...
  rewrite* subst_open. apply_fresh* para_mured as y. cross*.
...
Proof.
  ...
  applys~ (@star_trans beta (t2 ^^ trm_mu t1 t2)).
   pick_fresh x. apply* (@beta_star_red_through x).
   apply~ (@star_trans beta (trm_mu t1' t2)).
...
Proof.
  ...
  exists (trm_prod u1 p). split; 
   apply_fresh* para_prod as y; apply* (@para_red_rename x).
    (* case: mured / mured *)
  lets~ [u2 [U2a U2b]]: IHHS t1'0 __. clear IHHS.
  pick_fresh x. lets~ [u1x [U1a U1b]]: H0 (t2'0 ^ x) __. auto. clear H0.
  destruct~ (@close_var_spec u1x x) as [u1 [EQu1 termu1]].
  rewrite EQu1 in U1a, U1b.
