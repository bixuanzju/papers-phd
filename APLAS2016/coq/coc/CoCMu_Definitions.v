(***************************************************************************
* CoC + Mu + Type-in-type - Definitions                                    *
...
Inductive value : trm -> Prop :=
  ...
  | value_abs : forall t1 t2,
      term (trm_abs t1 t2) -> value (trm_abs t1 t2)
...
Inductive beta : relation :=
  ...
  | beta_prod2 : forall L t1 t2 t2', 
     ...
     (forall x, x \notin L -> beta (t2 ^ x) (t2' ^ x)) ->
     beta (trm_prod t1 t2) (trm_prod t1 t2')
  | beta_mured : forall t1 t2,
      term (trm_mu t1 t2) ->
      beta (trm_mu t1 t2) (t2 ^^ trm_mu t1 t2)
  | beta_mu1 : forall t1 t1' t2, 
     body t2 ->
     beta t1 t1' ->
     beta (trm_mu t1 t2) (trm_mu t1' t2)
  | beta_mu2 : forall L t1 t2 t2', 
     term t1 ->
     (forall x, x \notin L -> beta (t2 ^ x) (t2' ^ x)) ->
     beta (trm_mu t1 t2) (trm_mu t1 t2').
...
(** Progress for weak-head reduction *)
Definition progress_wh := forall t T, 
  typing empty t T ->
     value t 
  \/ exists t', t ~~> t'.
